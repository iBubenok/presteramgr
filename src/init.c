...
{
  GT_STATUS rc;
  CPSS_EXT_DRV_FUNC_BIND_STC ext_drv_funcs;
  CPSS_OS_FUNC_BIND_STC os_funcs;
  CPSS_TRACE_FUNC_BIND_STC trace_funcs;
  CPSS_DXCH_PP_PHASE1_INIT_INFO_STC ph1_info;
  CPSS_PP_DEVICE_TYPE dev_type;

  cpssGetDefaultOsBindFuncs (&os_funcs);
  cpssGetDefaultExtDrvFuncs (&ext_drv_funcs);
  cpssGetDefaultTraceFuncs (&trace_funcs);

  rc = cpssExtServicesBind (&ext_drv_funcs, &os_funcs, &trace_funcs);
  RCC (rc, cpssExtServicesBind);
