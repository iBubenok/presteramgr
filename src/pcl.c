...
static uint16_t port_ip_udp_mc_mirror_rule_ix[NPORTS + 1] = {};
...
initialize_vars (void)
{
  ...
  for_each_port(pid) {
    ...
    port_ip_udp_mc_mirror_rule_ix[pid]     = idx[port->ldev]++;
  }
...
/******************************************************************************/
/* UDP MULTICAST MIRROR                                                      */
/******************************************************************************/
static uint8_t udp_mc_dest_ip[4]      = {224, 0, 0, 0};
static uint8_t udp_mc_dest_ip_mask[4] = {240, 0, 0, 0};
void
pcl_setup_udp_mc(int d)
{
  port_id_t pi;
  for_each_port(pi) {
    struct port *port = port_ptr (pi);
    ...
    memcpy (&rule.ruleExtNotIpv6.dip, udp_mc_dest_ip, 4);
    memcpy (&mask.ruleExtNotIpv6.dip, udp_mc_dest_ip_mask, 4);

    act.pktCmd = CPSS_PACKET_CMD_MIRROR_TO_CPU_E;

    act.actionStop = GT_TRUE;
    act.mirror.cpuCode = CPSS_NET_FIRST_USER_DEFINED_E + 15;

    CRP (cpssDxChPclRuleSet
         (port->ldev,                                       /* devNum         */
          CPSS_DXCH_PCL_RULE_FORMAT_INGRESS_EXT_NOT_IPV6_E, /* ruleFormat     */
          port_ip_udp_mc_mirror_rule_ix[pi],                  /* ruleIndex      */
          0,                                                /* ruleOptionsBmp */
          &mask,                                            /* maskPtr        */
          &rule,                                            /* patternPtr     */
          &act));                                           /* actionPtr      */
...
pcl_setup_ripng(int d) 
{
  ...
  for_each_port(pi) {
    ...
    rule.ruleExtIpv6L4.dip.arIP[15] = 0x09;
    ...
    CRP (cpssDxChPclRuleSet
         (port->ldev,
          ...
          port_ipv6_ripng_mirror_rule_ix[pi],
          0,
          &mask,
          &rule,
          &act));
